...
function NewStoresLastMonth() {
    ...
    async function getNewStores(pageNo) {
        setLoading(true)
        const verifyObject = getTheToken()
        if (verifyObject.available) {

        const resp = await fetch(`${process.VITE_BASE_URL}/stores/lastmonthStores/${pageNo}`,{
            headers:{
                token:verifyObject.token
            }
        })
        const data = await resp.json()
        GetNewStoreDetails(data.data.array)
        setDocumentCount(data.data.newStoresCount)
        }
    }

    function GetNewStoreDetails(totalSale) {
    ...
    useEffect(() => { getNewStores(pageCount) }, [])
    ...
    return (
        <div className='totoal_stores_tabel'>
            ...
            {loading ?
                ...
                :
                <>
                    <h3>Total Stores Last Month: {documentCount}</h3>
...
export default NewStoresLastMonth
