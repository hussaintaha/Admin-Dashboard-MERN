...
function TotalStores() {
    ...
    async function getAllShops(pageNo) {
        ...
        if (verifyObject.available) {
            const resp = await fetch(`${process.VITE_BASE_URL}/stores/totalStores/${pageNo}`, { headers: { token: verifyObject.token } })
            const data = await resp.json();
            console.log("resp of totla stores ",data)
                                ...
                                {storesArray?.map((element, index) => {
                                    ...
                                    return (
                                        <tr key={element?.storeId}>
                                            {!element.status ?
                                                <td data-label="S.N" className='indexTD storeStatus storeStatusActive' >{((pageCount - 1) * 10) + index + 1}</td>
                                                :
                                                <td data-label="S.N" className='indexTD storeStatus storeStatusDeActive' >{((pageCount - 1) * 10) + index + 1}</td>
                                            }
